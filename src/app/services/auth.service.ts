...
export class AuthService {
  ...
  login(email: string, password?: string): Observable<ApiResponse<string>> {
    // Note: data is now string (the token)
    return this.http.post<ApiResponse<string>>(`${this.apiUrl}/auth/login`, { email, password })
      .pipe(
        tap(response => {
  ...
  register(email: string, firstName: string, lastName: string, role: UserRole = 'teacher') {
    const newUser: User = {
